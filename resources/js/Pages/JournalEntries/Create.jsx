...
export default function Create({ auth, accounts = [], fiscalYears = [], contacts = [], selectedFiscalYearId = null, defaultEntryDate = null }) {
    const selectedYear = fiscalYears.find(y => y.id == selectedFiscalYearId) || fiscalYears[0];
    const defaultDate = defaultEntryDate || (selectedYear ? selectedYear.start_date?.split('T')[0] : new Date().toISOString().slice(0, 10));
    
    const { data, setData, post, processing, errors } = useForm({
        entry_date: defaultDate,
        description: '',
        fiscal_year_id: selectedFiscalYearId || fiscalYears[0]?.id || '',
        lines: [
    ...
    const handleAccountInput = (index, value) => {
        ...
        const lines = [...data.lines];
        const matchedAccount = accounts.find(acc => `${acc.code} - ${acc.name}` === value);
        if (matchedAccount) {
            lines[index].account_id = matchedAccount.id;
        } else {
    ...
    const addLine = () => {
        ...
        const newLines = [...data.lines, { account_id: '', contact_id: '', description: '', debit: '', credit: '' }];
    ...
    const removeLine = (index) => {
        ...
        data.lines.forEach((_, i) => {
            if (i !== index) {
                newAccountSearch[newIndex] = accountSearch[i] || '';
                newShowDropdown[newIndex] = showDropdown[i] || false;
                newIndex++;
            }
        });
        setAccountSearch(newAccountSearch);
            ...
            <div className="min-h-screen bg-gray-50 pb-12" dir="rtl">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="mb-6 flex items-center justify-between">
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900">إنشاء قيد يومية جديد</h1>
                            <p className="mt-1 text-sm text-gray-600">تسجيل القيود المحاسبية اليدوية وتحديد الجهات</p>
                        </div>
                        ...
                        <form onSubmit={submit} className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 max-w-5xl mx-auto">
                                <div className="md:col-span-3 lg:col-span-3">
                                    ...
                                    />
                                    {errors.entry_date && <div className="text-red-500 text-sm mt-1">{errors.entry_date}</div>}
                                </div>
                                <div className="md:col-span-3 lg:col-span-3">
                                    ...
                                    {errors.fiscal_year_id && <div className="text-red-500 text-sm mt-1">{errors.fiscal_year_id}</div>}
                                </div>
                                                ...
                                                <td className="px-2 py-3">
                                                    <div className="relative" ref={el => dropdownRefs.current[index] = el}>
                                                        ...
                                                        {showDropdown[index] && getFilteredAccounts(index).length > 0 && (
                                                            <div 
                                                                className="absolute z-[9999] bg-white border border-gray-200 rounded-xl shadow-xl"
                                                                style={{ 
                                                                    width: '350px', 
                                                                    maxHeight: '200px',
                                                                    overflowY: 'auto',
