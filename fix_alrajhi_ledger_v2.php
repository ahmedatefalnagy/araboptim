...
DB::beginTransaction();
try {
    // 1. Fix JV-001306 (ID 1320) - Flip Debit/Credit IF Al-Rajhi is currently Debit
    $je1320 = JournalEntry::find(1320);
    if ($je1320) {
        $alRajhiLine = $je1320->lines()->where('account_id', 8)->first();
        if ($alRajhiLine && $alRajhiLine->debit > 0) {
            foreach($je1320->lines as $line) {
                $oldDebit = $line->debit;
                $line->update([
                    'debit' => $line->credit,
                    'credit' => $oldDebit
            ...
            echo "JV-001306 already correct or Al-Rajhi not found.\n";
        }
    }

    // 2. Fix Entry 38 (Date mismatch?)
    $je38 = JournalEntry::find(38);
    if ($je38 && $je38->entry_date->format('Y-m-d') != '2025-05-11') {
    ...
    foreach($missing as $m) {
        ...
        if(!$existing) {
            ...
            echo "Entry {$m['no']}-FIX already exists.\n";
        }
    }
